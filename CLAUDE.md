...
- `applications.env` is JSONB, overridden in sqlc to `map[string]string` (the deploy source — env is a set there). `env_text` (migration 000013, TEXT) stores the same vars as raw, ORDER-PRESERVING `KEY=VALUE` lines for the editor (the textarea shows `c.App.EnvText`, not a re-sorted map); both are written together by `saveEnv`/`createApp`. `parseEnv` returns the map **and** duplicate keys → `saveEnv` rejects dupes. `domain` has a UNIQUE index; `source_type` CHECK (`image`/`dockerfile`). Advanced container columns (migration 000008): nullable `memory_limit`/`cpu_limit` (TEXT, human units), `replicas` (default 1), `restart_condition` CHECK (`any`/`on-failure`/`none`), `restart_max_attempts`, and `healthcheck_*` (cmd/interval/timeout/retries/start_period). Nullable `command` (migration 000012, TEXT) is an optional container CMD override — `docker.SplitCommand` whitespace-splits it into `ServiceSpec.Args` (overrides the image CMD, keeps ENTRYPOINT; e.g. `start-dev` for Keycloak). Edited on the Advanced tab via `saveAdvanced`.
