...
Status: Phases 0, 1, 2, 2.5, 3, **4, 5** are DONE (login, image apps, Swarm services, Traefik routing, live logs, rolling updates, Dockerfile build+deploy, deploy history, UI design system + i18n, managed Postgres/Redis with volumes and external ports, multi/custom domains per app + per-domain Let's Encrypt HTTPS, **Postgres backups to S3/MinIO with cron + retention + restore**). Tech-debt backlog for Phases 1–3 is fully closed. Plus a **deploy-parity** track (gaps vs Dokploy for deploying a real app): **registry credentials (private image pull) is DONE**; still missing — advanced container settings (resources/replicas/restart/healthcheck), app volumes, DB→app `DATABASE_URL` injection, app stop/start, build secrets/private-git. The bot's HTTPS webhook + DB durability + private-image pull are unblocked; full deploy-parity is in progress.
