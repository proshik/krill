...
| `KRILL_ALLOW_PRIVATE_EGRESS` | `false` | Allow outbound connections to private, loopback and link-local addresses for S3 storage, registries and Git clones, and the [Observability](guides/observability.md) check. Off by default as SSRF protection; enable it for a MinIO, registry or Git server on your own network. |
