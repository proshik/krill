...
```
   ...
   CI      ───▶ │   - state in PostgreSQL                        │
                ...
                │   - observability agent  krill-alloy-node      │
                └───────────────────────┬────────────────────────┘
...
agent runs on the workers and no extra port is opened, unless you turn on the optional
observability agent below, which is itself a Swarm service rather than something installed on
the host.
...
**Settings → Observability** (instance-admin only) deploys `krill-alloy-node`, a global Swarm
service — one Grafana Alloy task per node — that ships host metrics (Prometheus remote write)
and every container's logs (Loki) to storage you provide. Krill stores only the two push
addresses and their credentials; it keeps none of the data itself. The agent needs the Docker
socket, so it stays off every organization's network and publishes no port. See the
...
| Operations | `notification_channels`, `metric_samples`, `observability_settings` |
