...
works) and puts a "new" badge on the Settings link in the sidebar when one is found.
...
a database or volume backup/restore, or a database instance's node move is running, so a
restart never cuts one short. The container image never self-updates — pull a new tag instead
(see [Run the container image instead](#run-the-container-image-instead)).
...
"requires re-running install.sh". Otherwise it is just another valid way to upgrade, and it
leaves a `krill.prev` and the update drop-in from a prior UI update in place.
...
place either way — and running `systemctl stop krill` inside the window has the same effect as
the timer firing. A unit hardened with `ProtectSystem=strict` (`install.sh` does not set this)
needs `ReadWritePaths=/usr/local/bin /etc/systemd/system /run` added, or the Updates page
reports the binary's directory as read-only and never offers an update.

**Manual:** the Updates page offers **Roll back to vP** whenever a `krill.prev` exists, no
timer involved. By SSH, the same swap:
